    ...
    >
      ...
      >
        ...
        <div className="p-6 text-center">
          ...
          {item.description && (
            <p className="text-gray-500 text-sm leading-relaxed bg-gray-50 p-3 rounded-lg border border-gray-100">
              {item.description}
            </p>
          )}
...
const EditModal: React.FC<{
  ...
  const handleDeleteClick = async () => {
    if (window.confirm(`Tem certeza que deseja EXCLUIR "${item.code}"?`)) {
      setLoading(true);
      try {
      ...
      <div className="bg-white w-full max-w-lg rounded-2xl shadow-2xl overflow-hidden">
        <div className="bg-[#313b2f] p-5 flex items-center justify-between">
          <h2 className="text-white font-bold text-lg flex items-center gap-2">
            <FaEdit className="text-[#ffd639]" /> Editar Aplique
          </h2>
          <button
            onClick={onClose}
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-4">
          ...
          <div
            className="flex items-center gap-2 p-3 bg-gray-50 rounded-lg border border-gray-200 cursor-pointer"
          ...
          >
            <input
              ...
              className="w-5 h-5 text-[#ffd639] rounded focus:ring-[#ffd639] cursor-pointer"
            />
  ...
  if (isLoading)
    return (
      <div className="flex flex-col items-center justify-center min-h-[50vh] text-gray-500 animate-pulse">
        <FaImage className="text-4xl text-[#ffd639] mb-4" />
        <p className="text-lg font-medium">Carregando Apliques...</p>
      </div>
    );
    ...
    <div className="w-full max-w-[1400px] mx-auto px-4 py-8 md:pt-32 pb-20">
      ...
      <div className="flex flex-col md:flex-row items-center justify-between gap-6 mb-10">
        <div className="text-center md:text-left">
          <h1 className="text-3xl md:text-4xl font-bold text-[#313b2f] mb-2">
            Catálogo de Apliques
          </h1>
          <p className="text-gray-500">
            {isStaff
              ? "Painel de Gestão: Clique para editar itens."
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 gap-4 md:gap-6">
        {apliquesFiltrados.map((item) => (
          ...
          >
            {/* Imagem */}
            <div className="aspect-square w-full p-4 flex items-center justify-center bg-gray-50/50">
              <img
                src={item.imageUrl}
                alt={item.code}
      ...
      {editingItem && (
        <EditModal
          ...
          onSave={updateAplique}
          onDelete={deleteAplique}
        />
      )}
